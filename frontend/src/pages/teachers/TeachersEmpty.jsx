...
// Per spec, email must be unique too (not just id) — case-insensitive,
// matching the backend's own lower(email_id) comparison.
function duplicateEmailError(emails) {
  const seen = new Set()
  for (const email of emails) {
    const normalized = email.trim().toLowerCase()
    if (!normalized) continue
    if (seen.has(normalized)) return resolveApiError({ error_code: ErrorCode.EMAIL_ALREADY_USED, context: { email } })
    seen.add(normalized)
  }
  return null
}
...
function validateRows(rows) {
  ...
  const dupIdError = duplicateIdError(extracted.map(r => r.org_id))
  if (dupIdError) return { error: dupIdError }
  const dupEmailError = duplicateEmailError(extracted.map(r => r.email))
  if (dupEmailError) return { error: dupEmailError }
...
function validateWorkbook(workbook) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    allEmails.push(...result.rows.map(r => r.email))
  }
  const dupIdError = duplicateIdError(allIds)
  if (dupIdError) return { error: dupIdError }
  const dupEmailError = duplicateEmailError(allEmails)
  if (dupEmailError) return { error: dupEmailError }
...
// id or email repeated ACROSS files, not just within one file's own sheets
// — the backend itself has no notion of "which file a row came from" (it
// just receives one flat `teachers` list), so this is the only place that
...
// list already had teachers before this upload (see handleFiles below) —
    ...
    <div className="teachers-empty">
      ...
      <div className="teachers-upload-row">
        ...
        <div
          ...
          role="button"
          tabIndex={0}
          aria-label="Drop teacher list file(s) here"
        >
        ...
        <div
          ...
          aria-label="Browse for teacher list file(s)"
        >
