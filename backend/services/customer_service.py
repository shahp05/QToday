...
_SELECT = (
    "SELECT c.customer_id, c.customer_name, c.customer_acronym, c.country_id, "
    "       c.customer_address, c.customer_city, c.customer_state_id, c.customer_zip, "
    "       st.state_name AS customer_state, "
    ...
    "LEFT JOIN states st ON st.state_id = c.customer_state_id "
...
def update_my_customer(db: Session, *, customer_id: int, editor_user_id: int, payload: CustomerUpdateRequest) -> dict:
    ...
    db.execute(
        text(
            ...
            "  customer_state_id = :state_id, customer_zip = :zip, customer_email = :email, "
        ...
        ),
        {
            ...
            "state_id": payload.customer_state_id,
